...
const server = createServer(async (request,response) => {
  try {
    ...
    if (request.method === "GET" && url.pathname === "/status") return reply(response,200,{chainId:chain.state.chainId,height:chain.state.height,latestBlock:chain.state.lastBlockHash,genesisTime:chain.state.genesisTime,protocolVersion:chain.genesis.protocolVersion,supply:{...chain.state.supply,totalNova:formatNova(chain.state.supply.total)},validatorCount:Object.keys(chain.state.validators).length,mempool:chain.mempool.length});
    ...
    if (request.method === "POST" && url.pathname === "/tx") { const tx=await body(request),txId=chain.submit(tx),block=chain.produceBlock(validatorKey);const receipt=block.receipts.find(item=>item.txId===txId);return reply(response,receipt?.success?202:400,{txId,blockHeight:block.header.height,blockHash:block.hash,receipt}); }
