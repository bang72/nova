...
test("RPC creates an account and finalizes a signed transfer", async (context) => {
  ...
  const port=43000+(process.pid%1000),base=`http://127.0.0.1:${port}`,child=spawn(process.execPath,[resolve("chain/src/node.mjs")],{cwd:resolve("."),env:{...process.env,NOVA_DATA_DIR:dir,NOVA_RPC_PORT:String(port)},stdio:["ignore","pipe","pipe"]});
