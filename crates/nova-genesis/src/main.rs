...
#[command(
    name = "nova-genesis",
    version,
    about = "Build and verify NOVA genesis manifests"
)]
struct Cli {
    #[command(subcommand)]
    command: Cmd,
...
fn verify(path: &str) -> anyhow::Result<()> {
    ...
    if g.chain_id.trim().is_empty() {
        bail!("chain_id is empty");
    }
    if g.accounts.is_empty() {
        bail!("genesis has no accounts");
    }
    ...
    for a in &g.accounts {
        ...
        total = total
            ...
            .context("supply overflow")?;
    }
    if total > MAX_SUPPLY_BASE {
    ...
    println!(
        "valid chain_id={} protocol_version={} accounts={} validators={} supply_base={}",
        g.chain_id,
        g.protocol_version,
        g.accounts.len(),
        g.validators.len(),
