...
impl ValidatorSet {
    pub fn new(validators: Vec<Validator>) -> Result<Self, ConsensusError> {
        if validators.is_empty() { return Err(ConsensusError::EmptySet); }
        let mut map = BTreeMap::new();
        let mut total = 0u128;
        for v in validators {
            if v.voting_power == 0 { return Err(ConsensusError::ZeroPower); }
            total += v.voting_power as u128;
            if map.insert(v.id, v).is_some() { return Err(ConsensusError::Duplicate); }
        }
        Ok(Self { validators: map, total_power: total })
    }

    pub fn total_power(&self) -> u128 { self.total_power }
    ...
    pub fn verify_quorum(&self, certificate: &FinalityCertificate) -> Result<(), ConsensusError> {
        ...
        for signer in &certificate.signers {
            if !seen.insert(*signer) { continue; }
            let v = self.validators.get(signer).ok_or(ConsensusError::UnknownSigner)?;
...
mod tests {
    use super::*;
    fn id(x: u8) -> AccountId { AccountId([x; 32]) }

    #[test]
    fn strict_two_thirds_quorum() {
        let set = ValidatorSet::new(vec![
            Validator { id: id(1), voting_power: 34 },
            Validator { id: id(2), voting_power: 33 },
            Validator { id: id(3), voting_power: 33 },
        ]).unwrap();
        let ok = FinalityCertificate { height: 1, block_hash: [9;32], signers: vec![id(1), id(2)] };
        assert!(set.verify_quorum(&ok).is_ok());
        let no = FinalityCertificate { height: 1, block_hash: [9;32], signers: vec![id(2), id(3)] };
